...
func (instance *EC2Instance) ExportModule() error {
    ...
    if instance.Role.TargetGroup != nil {
        provider = strings.Split(instance.Role.TargetGroup.VirtualFirewall.GetSubnet().Kubernetes.Cloud.GetCloudProvider().GetType(), "*kubeml.")[1]    
    } else {
        provider = strings.Split(instance.Role.VirtualFirewall.GetSubnet().Kubernetes.Cloud.GetCloudProvider().GetType(), "*kubeml.")[1]
    }
    ...
    instanceType := strings.Split(reflect.TypeOf(instance).String(), "*kubeml.")[1]
    subnetType := strings.Split(reflect.TypeOf(subnet).String(), "*kubeml.")[1]

    var scgType string
    if instance.Role.TargetGroup != nil {
        scgType = strings.Split(reflect.TypeOf(instance.Role.TargetGroup.VirtualFirewall).String(), "*kubeml.")[1]
    } else {
        scgType = strings.Split(reflect.TypeOf(instance.Role.VirtualFirewall).String(), "*kubeml.")[1]
    }

    module := terraform.NewModule(instance.ID, provider, strings.Split(reflect.TypeOf(instance).String(), "*kubeml.")[1])
    for i := 0; i < e.NumField(); i++ {
    ...
    if instance.IAMRole != nil {
        roleType := strings.Split(reflect.TypeOf(instance.IAMRole).String(), "*kubeml.")[1]
